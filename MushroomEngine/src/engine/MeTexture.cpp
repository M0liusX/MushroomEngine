...
namespace me {
   MeTexture::MeTexture(MeDevice& device, const std::string& textureFilepath) : mDevice{ device } {
   ...
   std::unique_ptr<MeTexture> MeTexture::createTextureFromFile(
      MeDevice& device, const std::string& filepath) {
      return std::make_unique<MeTexture>(device, filepath);
   }

   void MeTexture::updateDescriptor() {
      mDescriptor.sampler = mTextureSampler;
      mDescriptor.imageView = mTextureImageView;
      mDescriptor.imageLayout = mTextureLayout;
   }

   void MeTexture::createTextureImage(const std::string& filepath) {
   ...
   void MeTexture::createTextureImageView(VkImageViewType viewType) {
      VkImageViewCreateInfo viewInfo{};
      viewInfo.sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO;
      viewInfo.image = mTextureImage;
   ...
   void MeTexture::createTextureSampler() {
      VkSamplerCreateInfo samplerInfo{};
      samplerInfo.sType = VK_STRUCTURE_TYPE_SAMPLER_CREATE_INFO;
      samplerInfo.magFilter = VK_FILTER_LINEAR;
      samplerInfo.minFilter = VK_FILTER_LINEAR;
   ...
   void MeTexture::transitionLayout(
